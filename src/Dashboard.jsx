...
import { Truck, Users, FileText, AlertCircle, ArrowRight } from 'lucide-react';
import { useData } from '@/contexts/DataContext';

const Dashboard = ({ setActiveTab }) => {
    const dataApi = useData();
    const drivers = dataApi.getDrivers();
    const clients = dataApi.getClients();
    const orders = dataApi.getFreightOrders();
    const earningsConfig = dataApi.getEarningsConfig();
    const referralPayouts = dataApi.getReferralPayouts();

    const pendingVerifications = drivers.filter(d => d.status === 'pending').length + clients.filter(c => c.status === 'pending').length;
    const totalOrders = orders.length;
    const totalRevenue = orders.reduce((acc, order) => {
        if(order.status === 'completed' && order.value) {
    ...
    const pendingPayouts = referralPayouts.filter(p => p.status === 'pending').length;
