...
import { FileText, Package, DollarSign, Calculator, FileBadge, TrendingUp, Gift, Star, User, LogOut, Home, ArrowLeft, Menu, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useNavigate } from 'react-router-dom';
import { ScrollArea } from '@/components/ui/scroll-area';

const DriverLayout = ({ children, activeTab, setActiveTab, driverName, navItems }) => {
  const navigate = useNavigate();
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  const iconMap = {
    FileText, Package, DollarSign, Calculator, FileBadge, TrendingUp, Gift, Star, User
  };
    ...
    <>
      <div>
        <div className="mb-8 hidden md:block">
          <h1 className="text-2xl font-bold gradient-text-purple">Motorista</h1>
          <p className="text-sm text-gray-400 truncate">{driverName}</p>
        </div>
        <div className="flex md:hidden justify-between items-center mb-4">
           <h1 className="text-xl font-bold gradient-text-purple">Motorista</h1>
            ...
            {navItems.map(item => {
              ...
              return (
                <Button
                  ...
                  className={`w-full justify-start ${activeTab === item.id ? 'bg-purple-500/30 text-white' : 'text-gray-300 hover:bg-purple-500/10 hover:text-white'}`}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 p-4 md:p-8 overflow-y-auto">
          <div className="flex flex-wrap gap-2 mb-6">
            ...
            <Button variant="outline" size="sm" onClick={() => setActiveTab('quote-opportunities')} className="bg-white/10 border-white/20 hover:bg-white/20">
